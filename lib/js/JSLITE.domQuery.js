...
* @return {Array}
* @describe <p>Basic DOM CSS Selector engine. Returns an array of HTMLElements.</p><p>Though it's possible to invoke this as a class method, it's preferable to invoke <code><a href="#jsdoc">JSLITE.Element.gets</a></code> as this will return a composite element.</p> 
<p>
...
var cLinks = JSLITE.Element.gets("a[href]");
cLinks.addClass("grayBackground");

//find all links that are descendants of divs and disable them;
var cLinks = JSLITE.Element.gets("div a[href]");
cLinks.disable();
 
//find elements with multiple class names (the order of the class names isn't important);
var cElems = JSLITE.Element.gets("div ul.topNav li.home.external");

//fine-tune your searches using a dose of classes and ids and attribute selectors;
var cElems = JSLITE.Element.gets("div#header.page p span#about");

or

//find all anchors that have both an href and a rel attribute within any list item that has a class, etc.;
var cLis = JSLITE.Element.gets("body div[style].zucchero#theDiv.boo ul li[class=foo] a[rel][href]");
etc.

//collect all form elements for a form submission;
var cElems = JSLITE.Element.gets("#theForm asterisk[name]"); //NOTE replace the word "asterisk" with the actual symbol;

//attribute filters;
JSLITE.Element.gets("body div[class=zucchero boo foobar] ul[id^=someId] li[id][class=blue]");

JSLITE.Element.gets("div li[class*=too]");
*/
//<source>
    ...
    fnSearch = function (sSelector, oRoot) {
      ...
      aSelector.forEach(function (sSelector) {
        aPassed.push((function () {
          ...
          oRoot = oRoot ? JSLITE.Element.get(oRoot, true) : document;
          sBase = aChunks.pop();
